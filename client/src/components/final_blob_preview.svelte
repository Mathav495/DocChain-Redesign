...
<div class="flex w-full flex-col">
  {#if (src = blobimage)}
    <img
      class="block shadow-[0_0_8px_0_rgba(0,0,0,0.15)] rounded-md w-full "
      src={blobimage}
      alt="document"
    />
  {:else if loading}
  ...
  {:else}
    <div class="  flex max-w-full flex-col rounded-md">
      <canvas
        id="canv"
        class="border-2 block rounded-md overflow-hidden max-w-full  shadow-[0_0_8px_0_rgba(0,0,0,0.15)] aspect-auto"
      />
      <div
        class="flex justify-center items-center abolute bottom-0 inset-x-0 gap-3"
      >
